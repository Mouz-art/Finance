...
@app.route("/")
def dashboard():
    total_contrib = db.session.query(func.coalesce(func.sum(Contribution.amount), 0)).scalar()
    ...
    recent_contribs = Contribution.query.order_by(Contribution.date.desc()).limit(5).all()
...
def contributions_list():
    ...
    total_contrib = db.session.query(func.coalesce(func.sum(Contribution.amount), 0)).scalar()
    return render_template("contributions.html", contribs=contribs, total_contrib=total_contrib or 0)
...
def event_detail(event_id):
    ...
    transactions = EventTransaction.query.filter_by(event_id=event_id).order_by(EventTransaction.date.desc()).all()
